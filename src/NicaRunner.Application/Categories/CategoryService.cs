...
public class CategoryService(ICategoryRepository categoryRepository, IRunnerRepository runnerRepository) : ICategoryService
{
    ...
    public async Task<CategoryDto> UpdateAsync(int categoryId, UpdateCategoryRequest request, CancellationToken ct = default)
    {
        var category = await GetCategoryOrThrowAsync(categoryId, ct);
        EnsureValidAgeRange(request.EdadMinima, request.EdadMaxima);
        await EnsureCodigoIsUniqueAsync(request.Codigo, categoryId, ct);

        category.Codigo = request.Codigo.Trim().ToUpperInvariant();
